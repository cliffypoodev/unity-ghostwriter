...
RESOLUTION TEXTURE RULE (ALL GENRES): No scene of acceptance, recognition, victory, or belonging should be unanimous. At least one element must contain friction: a character who withholds full approval, a condition attached to the acceptance, a cost named but not yet paid, or a single dissenting detail in the environment. This is not conflict — it is texture. It makes the resolution feel earned rather than given. Minimum: one line or beat per resolution scene that complicates the acceptance without derailing it. BAD: "Everyone accepts Marcus. The community welcomes him. Zephyr is proud. All is well." GOOD: "The council accepts Marcus. One elder near the back does not raise his voice with the others. Zephyr notices. Marcus does too." — the elder becomes a thread, not a detour.`;
