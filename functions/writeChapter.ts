...
function checkSceneEnding(text){const paragraphs=text.trim().split(/\n\n+/);const lastP=paragraphs[paragraphs.length-1].trim();const lastS=lastP.split(/(?<=[.!?])\s+/).pop().trim();const weak=[/\b(he|she|they) (had |finally )?found (his|her|their) (place|home|peace|belonging)/i,/for the first time (in years|in his life|in her life|in their life)/i,/everything (had |would )?changed?/i,/this was (who|where|what) (he|she|they) (truly was|belonged|needed|wanted)/i,/(he|she|they) knew exactly where (he|she|they) belonged/i,/\?$/,/(he|she|they) felt (more |truly |finally )?(himself|herself|themselves|at peace|complete|whole|alive|ready)/i,/(he|she|they) (understood|realized|knew) (now |then )?(what|that|how|why)/i,/(he|she|they) was exactly where (he|she|they) (was|were) (supposed|meant) to be/i,/it was (going to be |finally )?(okay|alright|all right|enough)/i,/(he|she|they) (had|was|were) (no longer|not the same|finally free|at last free)/i];for(const p of weak){if(p.test(lastS))return{violation:true,lastSentence:lastS,rule:'Scene ending must close on image, action, dialogue, or sensory detail — not emotional summary or realization.'};}return{violation:false};}
const SPICE_LEVELS = {
...
const GEMINI_NF_ENFORCEMENT = `=== GEMINI NONFICTION ENFORCEMENT ===\nMin 2,500 words/chapter. Min 625 words per part. Write like Larson/Grann: cinematic first, analytical second.\nGEMINI NF BANS:\n1. Academic register as default — narrative nonfiction is cinematic, not bloodless. Open with a person in a moment.\n2. Over-hedging — "It could perhaps be argued that this may have contributed" → STATE THE CLAIM or flag [VERIFY].\n3. Padding to length — descriptive layers and qualifying clauses that repeat what was stated are padding. Cut them.\n4. Burying cinematic in analytical — do not front-load 3 paragraphs of context before the reader meets a human.\n5. Balanced-to-a-fault — "On the other hand... however... nevertheless" in same paragraph. Pick a position.\n6. "It is worth noting"/"One cannot overstate" — BANNED. State the thing directly.\n=== END GEMINI NF ===`;
