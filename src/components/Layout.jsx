        ...
        <div className="max-w-[1200px] mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            >
              ...
              <div className={`w-9 h-5 rounded-full transition-colors relative ${isMobileMode ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${isMobileMode ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </div>
            </button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="p-2.5 -mr-1 hover:bg-slate-100 active:bg-slate-200 rounded-lg transition-colors touch-manipulation" aria-label="Menu">
                  <Menu className="w-5 h-5 text-slate-600" />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={() => window.dispatchEvent(new CustomEvent('saveProject'))}>Save</DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to={createPageUrl("Settings")}>Settings</Link>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => window.dispatchEvent(new CustomEvent('deleteProject'))} className="text-red-600">Delete Project</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
