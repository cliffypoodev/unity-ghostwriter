...
async function callAI(modelKey, systemPrompt, userMessage, options = {}) {
  ...
  if (provider === "google") {
    ...
    const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + modelId + ':generateContent?key=' + apiKey, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: userMessage + '\n\nIMPORTANT: Return ONLY a valid JSON object. No markdown, no code fences, no commentary.' }] }], systemInstruction: { parts: [{ text: systemPrompt }] }, generationConfig: { temperature, maxOutputTokens: maxTokens, responseMimeType: 'application/json', thinkingConfig: { thinkingBudget: 0 } } }) });
