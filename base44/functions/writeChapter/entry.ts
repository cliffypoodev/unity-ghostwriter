...
function resolveModel(callType, spec) { try { if(!callType){console.warn('resolveModel called without callType — defaulting to claude-sonnet');return spec?.writing_model||spec?.ai_model||'claude-sonnet';} const H={outline:'gemini-pro',beat_sheet:'gemini-pro',post_gen_rewrite:'claude-sonnet',consistency_check:'claude-sonnet',style_rewrite:'claude-sonnet',character_interview:'claude-sonnet',metadata_generation:'claude-sonnet',sfw_handoff_check:'claude-sonnet',post_explicit:'claude-sonnet',cover_prompt:'claude-sonnet',keyword_generation:'claude-sonnet',kdp_description:'claude-sonnet',chapter_state:'claude-sonnet',consultant_chat:'claude-sonnet'}; if(H[callType])return H[callType]; if(callType==='explicit_scene')return 'deepseek-chat'; if(callType==='sfw_prose')return spec?.writing_model||spec?.ai_model||'claude-sonnet'; console.warn(`resolveModel: unknown callType "${callType}" — defaulting to claude-sonnet`); return 'claude-sonnet'; } catch(e){console.warn('resolveModel error:',e.message);return 'claude-sonnet';} }
...
const MODEL_MAP = {
  ...
  "gemini-flash":      { provider: "google",    modelId: "gemini-2.5-flash-001",     defaultTemp: 0.72, maxTokensLimit: null },
...
const FICTION_ENDING_TYPES={1:"Type A: Mid-action cliffhanger.",2:"Type B: Revelation recontextualizes.",3:"Type C: Concrete sensory image.",4:"Type D: Gut-punch dialogue — last line.",5:"Type E: Quiet mundane action contrasting intensity."};const NONFICTION_ENDING_TYPES={1:"Quiet resonant image.",2:"Reframing sentence.",3:"Brief poem/aphorism.",4:"Lingering question.",5:"Return to opening vignette."};function getBeatStyleInstructions(key){if(!key)return"Not specified";const beat=BEAT_STYLES[key];return beat?`${beat.name}\n${beat.instructions}`:key;}
function buildFictionProseRules(spec){const bs=spec?.beat_style||spec?.tone_style||'';const bn=bs?getBeatStyleInstructions(bs).split('\n')[0]:'Not specified';const vi=spec?.author_voice||'basic';const vn=vi!=='basic'?vi.split('-').map(w=>w[0].toUpperCase()+w.slice(1)).join(' '):'Basic — clean and competent';const li=parseInt(spec?.language_intensity)||0;return`════════════════════════════════\nFICTION PROSE RULES — MANDATORY\n════════════════════════════════\nRULE 1 — ABSOLUTE BANS (zero tolerance): "the particular [noun] of" | "something that looked/felt/seemed/sounded like" | "a kind of [noun] that" | "what might have been [smile/laugh/sob]" | "something shifted/loosened/cracked/tightened in chest" | "the weight/smell/sound/feel of everything" | "she/he realized that [emotional summary]" | "it was the kind of [silence/moment/look] that"\nRULE 2 — CAPS: "careful(ly)" ≤3 | "as if" ≤4 | "something in" ≤3 | "the kind of" ≤3 | "somehow" ≤2 | "familiar" ≤3 | "particular" ≤3 | "antiseptic" ≤1/ms | "fluorescent" ≤2/ms | "shifted in her/his" ≤1/ms\nRULE 3 — SCENE ENDINGS: Final sentence before * * * or chapter end must NOT be emotional summary. BANNED: "She thought maybe…"/"It felt like the beginning…"/"He knew nothing would be the same"/"She realized"/"He understood"/"And that felt like". USE ONE: physical action, concrete sensory detail, unadorned dialogue, visible image.\nRULE 4 — BEAT STYLE: ${bn}. Apply its specific rhythm, pacing, structure. Not generic. Not blended.\nRULE 5 — VOICE: ${vn}. Must be detectable in sentence rhythm, dialogue style, interiority, pacing, what narrator notices.\nRULE 6 — LANGUAGE Level ${li}: 0=none|1=mild sparingly|2=strong at spikes|3=danger/anger/shock|4=harsh earned by context. Never decorative. Never stacked.`;}
function buildCtxHeader(s){const bk=s?.beat_style||s?.tone_style||'';const bn=bk?getBeatStyleInstructions(bk).split('\n')[0]:'Not specified';const sp=parseInt(s?.spice_level)||0;const ln=parseInt(s?.language_intensity)||0;const SN={0:'Fade to Black',1:'Closed Door',2:'Cracked Door',3:'Open Door',4:'Full Intensity'};const LN={0:'Clean',1:'Mild',2:'Moderate',3:'Strong',4:'Raw'};const vi=s?.author_voice||'basic';const vn=vi!=='basic'?vi.split('-').map(w=>w[0].toUpperCase()+w.slice(1)).join(' '):'';return`═══ PROJECT CONTEXT ═══\nTYPE: ${(s?.book_type||'fiction').toUpperCase()} | GENRE: ${s?.genre||'Fiction'}${s?.subgenre?' / '+s.subgenre:''} | BEAT: ${bn} | LANG: ${ln}/4 ${LN[ln]||'Clean'}${sp>0?' | SPICE: '+sp+'/4 '+SN[sp]:''}${vn?' | VOICE: '+vn:''}${s?.target_audience?' | AUDIENCE: '+s.target_audience:''}\nPhase 1 settings MANDATORY.\n═══════════════════════`;}
...
const QUALITY_UPGRADES = `INTERIORITY RULE: Internal monologue max 2 consecutive sentences before returning to action/dialogue/sensory. Show the next thing they DO.\nDIALOGUE SUBTEXT: Every exchange >2 lines must contain subtext. Direct on-the-nose dialogue max 1x/chapter at emotional climax.\nSCENE/CHAPTER ENDING: Final paragraph ends on ONE of: physical image, dialogue, concrete action, specific sensory detail. NOT: emotional summary, stated realization, thematic declaration, rhetorical question. BAD: "He was exactly where he was supposed to be." GOOD: "He left the curtains open. Didn't check the door."\nOPENING: First sentence mid-action/sensation/dialogue/detail. No character name in first 5 words. No weather unless central conflict.\nSENSATION SPECIFICITY: Each sensation must be unique to THIS character in THIS moment. Anchor to: specific body location, character-specific comparison, something different from last sensation. BAD: "Her heart pounded." GOOD: "Her heart was doing something complicated — closer to the feeling before her name was called at a competition."\n`;
...
function isGeminiModel(modelKey) { return /^gemini/.test(modelKey || '') || (MODEL_MAP[modelKey]?.provider === 'google'); }
...
function _buildNonfictionSystemPrompt(spec, chapter_info, total_chapters, target_words,
  ...
  return `${_nfLen}${_dsBlock}${_nfBans}═══ PROJECT CONTEXT ═══\nTYPE: NONFICTION | GENRE: ${spec.genre || 'General'}${spec.subgenre ? ' / ' + spec.subgenre : ''} | BEAT: ${getBeatStyleInstructions(spec.tone_style || spec.beat_style).split('\\n')[0]} | LANG: ${parseInt(spec.language_intensity)||0}/4\nPhase 1 settings MANDATORY.\n═══════════════════════\n\nAUTHOR MODE — NONFICTION PROSE GENERATION
...
${_beatUsrBlock(chapterBeat)}`;
    ...
    // ── Structural validation via modelAdapter (Section 5 integration) ──
    try { const sv = await base44.functions.invoke('modelAdapter', { action: 'validate_full', text: fullContent, model_id: modelKey, chapter: { number: chapter.chapter_number }, chapter_number: chapter.chapter_number }); const svd = sv.data || sv; if (svd?.warnings?.length) svd.warnings.forEach(w => { if (!qualityResult.warnings.includes(w)) qualityResult.warnings.push(w); }); if (svd?.errors?.length) { svd.errors.forEach(e => qualityResult.warnings.push(`STRUCTURAL: ${e}`)); qualityResult.violation_count += svd.errors.length; } qualityResult.structural = { valid: svd?.valid, partCount: svd?.metrics?.partCount || 0, wordCount: svd?.metrics?.wordCount || 0, needsRetry: svd?.needsRetry, retryReason: svd?.retryReason }; console.log(`Ch ${chapter.chapter_number} structural:`, qualityResult.structural); } catch (svErr) { console.warn(`Structural validation skipped:`, svErr.message); }
    qualityResult.genAttempts = genAttempts;
    console.log(`Chapter ${chapter.chapter_number} quality scan (${genAttempts} attempt${genAttempts > 1 ? 's' : ''}):`, qualityResult);
    ...
    if (chapterIndex < 20) { try { await base44.functions.invoke('consistencyCheck', { project_id: projectId, chapter_id: chapterId, chapter_text: finalContent.slice(0, 6000) }); } catch (ccErr) { console.warn('Consistency check failed (non-blocking):', ccErr.message); } }
  } catch (err) {
